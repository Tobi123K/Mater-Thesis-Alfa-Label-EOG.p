...
# Load simulated data
table = pd.read_csv('EOG01_blink_preprocessed.txt',sep='\t',names=['amplitude','amplitudeB']).reset_index().rename({'index':'time'},axis=1)

####==================(KK)
#data = pd.read_csv("EOG05_blink_preprocessed.txt",sep=";",names=["t","amplitude","amplitudeB"])
#table = data[['amplitude','amplitudeB']]

####==================(KK)
table.amplitude.plot()

def detect_states(signal, begin=0.05, end=0.05):
...
states_up = detect_states(table.amplitude, 0.1, 0.01)
...
#print(total)
n,time = count_durating_states(states_up, 100,500)
print(n,time)
...
###Licence https://github.com/BMClab/BMC/blob/master/LICENSE.txt #####
...
def _plot(x, mph, mpd, threshold, edge, valley, ax, ind):
    ...
    try:
		...
		import pandas as pd
		%matplotlib inline
		data = pd.read_csv('EOG01_blink_preprocessed.txt', header = None, names=['p1'])
		data.head()
		
		data.size
		
		# INSERT TIME TABLE, CHANGE DATA SIDE
		t1 = np.arange(1,630339,1)
		t1.size
		
		data.insert(1,"t", True)
		
		peaks3 = detect_peaks(data["p1"][10000:630339], show=True, mpd=500, mph=0.17 )
